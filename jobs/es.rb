...
def get_eu_orders_cumulative(period, country)
  ...
  WHERE sku NOT LIKE 'ES-JBX%' AND sku NOT LIKE 'GIFTCARD%' AND o.status IN ('processing','pending','complete','closed') AND " + filter + ";"
...
def update_tick_uk_w
  ...
  #orders
  send_event('orders_uk_w_month', { current: get_eu_orders_cumulative(1,'uk') })
...
def update_tick_es_w
  ...
  #orders
  send_event('orders_es_w_month', { current: get_eu_orders_cumulative(1,'es') })
